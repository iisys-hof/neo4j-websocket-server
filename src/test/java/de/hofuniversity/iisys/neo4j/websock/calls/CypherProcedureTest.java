...
/**
 * Tests the stored procedure implementation for Cypher queries.
 */
public class CypherProcedureTest
{
    private static final String PROC_NAME = "procedure";
    ...
    private GraphDatabaseService setupService()
    {
    ...
    {
        GraphDatabaseService db = setupService();
        ...
        CypherProcedure procedure = new CypherProcedure(PROC_NAME, db, query);

        //call and check result
        Map<String, Object> parameters = new HashMap<String, Object>();
        parameters.put(ID_PARAM, JOHN_ID);
        parameters.put(WebsockConstants.SUBSET_START, 1);
        parameters.put(WebsockConstants.SUBSET_SIZE, 1);
        AResultSet<?> set = procedure.call(parameters);
    ...
    /**
     * Tests the procedure's reaction to a faulty Cypher query.
     */
    @Test
    public void testFaultyQuery()
    {
        ...
        //build faulty query procedure
        query = "START n=node:" + INDEX_NAME + "(" + ID_FIELD + "={"
            + ID_PARAM + "})\n"
            + "MATCH n-" + Neo4jRelTypes.ACTED.toString() + "->a\n"
            + "RETURN a, a." + ID_FIELD + ", a." + TITLE_FIELD + ";";
        procedure = new CypherProcedure(PROC_NAME, db, query);
